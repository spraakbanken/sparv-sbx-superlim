...
@exporter(
    "Saves sentence-wise predictions to a .tsv for further analysis", language=["swe"]
)
def predictions(
    source_files: AllSourceFilenames = AllSourceFilenames(),
    annotation_source_sentences: AnnotationAllSourceFiles = AnnotationAllSourceFiles(
        "<sentence>"
    ),
    annotation_migration_stance: AnnotationAllSourceFiles = AnnotationAllSourceFiles(
        "<sentence>:sbx_superlim.migration_stance"
    ),
    annotation_nuclear_stance: AnnotationAllSourceFiles = AnnotationAllSourceFiles(
        "<sentence>:sbx_superlim.nuclear_stance"
    ),
    annotation_nuclear_certainty: AnnotationAllSourceFiles = AnnotationAllSourceFiles(
        "<sentence>:sbx_superlim.nuclear_stance.certainty"
    ),
    ...
    for sf in source_files:
        ...
        text = Text(sf).read()
        sentences = [
            text[start:end] for start, end in annotation_source_sentences.read_spans(sf)
        ]
    ...
    pd.DataFrame.from_records(
        rows, columns=["source_file", "party", "year", "score", "n_sents"]
    ).sort_values("source_file").to_csv(out, sep="\t", index=False)
...
def swenli_parallel(
    ...
    logger.debug("source_files=%s", list(source_files))
    pairs = pair_files(source_files)
    logger.debug("pairs=%s", list(pairs))
    ...
    for sf_sv, sf_en in pairs:
        ...
        with open(f"{prefix}/{sf_sv}.txt") as f1, open(f"{prefix}/{sf_en}.txt") as f2:
            ...
            pipe = pipeline(
                "text-classification", model=hf_model_path, batch_size=hf_batch_size
            )
            output = pipe(pair_inputs, batch_size=hf_batch_size)
            label_mapper = get_label_mapper(ds_config, pipe.model.config)
            base = sf_sv.split(".")[0]
            pair_predictions[f"{base}.label"] = [
                label_mapper[o["label"]] for o in output
            ]
...
def swepar_parallel(
    ...
    for sf_sv, sf_en in pairs:
        ...
        with open(f"{prefix}/{sf_sv}.txt") as f1, open(f"{prefix}/{sf_en}.txt") as f2:
            ...
            model_outputs = model(
                **tokenizer(
                    pair_inputs, return_tensors="pt", truncation=True, padding=True
                )
            )
